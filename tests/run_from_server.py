...
from namebump import Client, Keypair, DONT_BUMP, THROW_BUMP
...
from namebump.server import Server, verified_pruning
...
async def NB_get_test_client_serv(
...
class TestPNPFromServer(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_NB_prune(self):
        ...
        await clients[IP6].put(NB_TEST_NAME, NB_TEST_VALUE, NB_LOCAL_SK)
        ...
        await clients[IP6].put(NB_TEST_NAME + b"2", NB_TEST_VALUE, NB_LOCAL_SK)
    ...
    async def test_NB_insert_fetch_del(self):
        ...
        for af in VALID_AFS:  # VALID_AFS
            ...
            # Do insert.
            await clients[af].put(NB_TEST_NAME, NB_TEST_VALUE, NB_LOCAL_SK)
            ...
            # Do update.
            await clients[af].put(NB_TEST_NAME, NB_TEST_VALUE + b"changed", NB_LOCAL_SK)
    ...
    async def test_NB_migrate_name_afs(self):
        ...
        for af_x in VALID_AFS:
            ...
            await clients[af_x].put(NB_TEST_NAME, NB_TEST_VALUE, NB_LOCAL_SK)
            ...
            for af_y in VALID_AFS:
                ...
                # Do the migration.
                await clients[af_y].put(NB_TEST_NAME, NB_TEST_VALUE, NB_LOCAL_SK)
...
New-NetIPAddress -InterfaceIndex 4 -IPAddress "fe80:3456:7890:3333:0000:0000:0000:0001" -PrefixLength 128 -AddressFamily IPv6 -Type Unicast
    ...
    async def test_NB_v6_range_limits(self):
        ...
        clients, serv = await NB_get_test_client_serv(
            v6_serv_ips="fe80:3456:7890:1111:0000:0000:0000:0001", i=i
        )
        ...
        for offset in range(0, len(vectors)):
            ...
            async def get_dest_pipe():
                ...
                pipe = await Pipe(TCP, client.addr, route).connect()
    ...
    async def test_v6_no_name_ip_prune(self):
        ...
        i = await Interface()
        clients, serv = await NB_get_test_client_serv(
            v6_serv_ips="fe80:3456:7890:1111:0000:0000:0000:0001", v6_name_limit=10, i=i
        )
